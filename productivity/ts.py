...
if len(sys.argv) < 2: # Error if the user doesn't provide a csv file
...
# Global variables
#csv_file = '~/bin/troubleshooting_servers.csv'
        ...
        while True:
            ...
            if key == curses.KEY_BACKSPACE or key == 127:
                ...
                if len(filtered_data) > 0:
                    ...
                    command_to_run = f"~/bin/trbl-tmux.sh {selected_row.iloc[3]} {selected_row.iloc[1]}" # SSH command using the first fieldk
                    try:
                        stdscr.addstr(len(filtered_data) + 2, 0, f"Troubleshooting: {selected_row.iloc[0]}")
                    except curses.error:
                        pass  # Handle error if the string cannot be displayed
                    stdscr.refresh()
